...
def get_first_half_of_string(string)
  string[0, ((string.length.to_f / 2).round)]
end
...
def average_of_array(array)
  ((array.inject {|sum, x| sum + x }).to_f/ array.length).round
end
...
def convert_array_to_a_hash(array)
  array_pairs = []
  array.each_slice(2) {|x| array_pairs.push(x)}
  array_pairs.to_h
end
